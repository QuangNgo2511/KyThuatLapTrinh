...
{
	soSV = 0;
	FILE* fp = fopen("D:/KyThuatLapTrinh/BaiThucHanh3/DanhSachSV.txt", "rt"); // Mo file van ban de doc
	if(fp == NULL) 
...
int main()
{
	SinhVien sv[100];
	int soSV = 0;
	DocDSSV("D:/KyThuatLapTrinh/BaiThucHanh3/DanhSachSV.txt", sv, soSV);
	printf("Danh sach sinh vien doc tu file:\n");
	InDSSV(sv, soSV);
	// Sap xep sinh vien DTB giam dan 
	SapXep(sv, soSV);
